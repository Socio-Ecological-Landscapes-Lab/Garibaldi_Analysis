...
dataset <-read_xlsx("/Users/lizmccleary/Desktop/Garibaldi_Analysis-1/ITEX_2025.xlsx")%>% 
  group_by(SITE,DATE,TRTMT,PLOT,X,Y,SPP)%>% 
...
coverTopBottom <- allDataTopBottom%>% 
  select(SITE, TRTMT, PLOT, DATE, SPP, HitOrder)%>% 
  mutate(SPP=as.factor(SPP))%>% 
  group_by(DATE, SITE, TRTMT, PLOT, SPP) %>% 
  tally(name = "cover") %>%
  ungroup()
...
ggplot(percent_cover_summary, aes(x = DATE, y = TotalPercentCover, color = SPP)) +
  geom_line() +
  geom_point() +
...
SITES<- coverTopBottom %>% 
        ...
        filter(!SPP %in% "carspp")%>%
        select(DATE,SITE,TRTMT,PLOT, SPP, cover)%>% 
...
Height_ALL <- dataset %>%
  ...
  select(DATE, SITE, TRTMT, PLOT, SPP, HitOrder, CanopyHeight.mm.)
...
Height_ALL$DATE <- as.numeric(Height_ALL$DATE)

#Specify dplyr package for group_by and summarize functions 
